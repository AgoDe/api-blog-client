...
const props = defineProps({
  ...
  disableClick: {
    type: Boolean,
    default: false,
    description: 'Disables submenu click on 2+ level when we are in horizontal and hover mode'
  }
})
  ...
  <ul :class="classContainer">
    ...
    >
      ...
      <div v-if="!node.heading && !node.sub" @click="linkClicked($event)">
        ...
        <a v-else :href="node.to" class="nav-main-link" :target="node.target || null">
          <i v-if="node.icon" :class="`nav-main-link-icon ${node.icon}`"></i>
          <span v-if="node.name" class="nav-main-link-name">
            {{ $t('menu.' + node.name) }}
          </span>
